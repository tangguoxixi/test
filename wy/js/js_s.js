$.ajax({
	...
	success: function(data) { //请求成功执行
		...
		$.each(data[0].main.blk2, function(i, t) {
			$("<li class='bl_li1'>"+"<span class='blk_li blk_li2'>"+t.num+"</span>"+"<a href='#' class='t_a' style='color: #333;' title='"+t.txt+"'>"+t.txt+"</a></li>").appendTo($(".ol1"));
		})
		$.each(data[0].main.blk3, function(i, t) {
			$("<li class='bl_li1'>"+"<span class='blk_li blk_li3'>"+t.num+"</span>"+"<a href='#' class='t_a' style='color: #333;' title='"+t.txt+"'>"+t.txt+"</a></li>").appendTo($(".ol2"));
		})
	},
