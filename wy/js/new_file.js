$.ajax({
	...
	success: function(data) { //请求成功执行
		...
		$.each(data[0].header[0], function(i, t) {
			$(".header-top-nav-ul").append("<a class='header-top-a'><li class='header-top-li1'>" + t[0] + "</li></a>")
			for(var i = 1; i < t.length; i++) {
				$(".header-top-nav-ul").append("<a class='header-top-a'><li class='header-top-li'>" + t[i] + "</li></a>")
			}
		})
		/*hcn*/
		$.each(data[0].header[1], function(i, t) {
			...
			for(var i = 1; i < t.length - 1; i++) {
				$(".header-center-ul").append("<a class='header-center-a'><li class='header-center-li'>" + t[i] + "</li></a>")
			}
			$(".header-center-ul").append("<a class='header-center-a'><li class='header-center-li1'>" + t[5] + "</li></a>")
		})
		/*mlt*/
			...
			$.each(data[0].nmain[1], function(i, a) {
				$.each(data[0].nmain[2], function(i, b) {
					for(var i = 0; i < t.length; i++) {
						var box1 = $("<li class='main-top-li'></li>").append($("<img />").attr("src", t[i].url)).append($("<p class='main-top-p'><a class='main-top-a'>" + a[i] + "</a></p>").prop("title", a[i])).append($("<p class='main-top-p1'><a class='main-top-a1'>" + b[i] + "</a></p>").prop("title", b[i])).append($("<a><div class='main-top-bg tc'></div></a>").prop("title", a[i]))
		...
		$.each(data[0].nmain[3], function(i, t) {
			$.each(data[0].nmain[4], function(i, a) {
				$.each(data[0].nmain[5], function(i, b) {
					for(var i = 0; i < t.length; i++) {
						var box2 = $("<li class='main-center-li'></li>").append($("<img />").attr("src", t[i].url)).append($("<p class='main-center-p'><a class='main-center-a'>" + a[i] + "</a></p>").prop("title", a[i])).append($("<p class='main-center-p1'><a class='main-center-a1'>" + b[i] + "</a></p>").prop("title", b[i])).append($("<a><div class='main-center-bg tc'></div></a>").prop("title", a[i]))
...
$(".header-center-a:eq(2)").attr("href", "index_s.html")
$(".header-center-a:eq(4)").attr("href", "singer.html")
$(function() {
...
$('.header-center-li:eq(1)').click(function() {
	alert("尚未开发")
})
$('.header-center-li:eq(3)').click(function() {
	alert("尚未开发")
})
...
cor.appendTo(".header-top-nav-ul")